...
CFD = Path(os.path.expanduser("~/.claude/tools/ingress-helper/ingress-helper.exe"))


def start_ingress(port):
    """
    Put the roamer's WebSocket on the public internet.

    Object storage is not a stream - the site could only ever poll it - so the
    live screen needs a socket a browser can open. A quick ingress gives one
    ...
    if not CFD.exists():
        say("no ingress-helper - the public feed stays on the slow path")
        return

    proc = subprocess.Popen(
        [str(CFD), "ingress", "--no-autoupdate", "--url",
    ...
    def watch():
        for line in proc.stdout:
            m = _re.search(r"https://[a-z0-9-]+\.trypublic-dns\.com", line)
            if m and not TUNNEL["url"]:
                TUNNEL["url"] = m.group(0)
                say("ingress open:", TUNNEL["url"])
...
def publish_address():
    """
    Tell the world where the ingress is.

    The quick ingress's address is random and changes every run, so the page
...
def publish(stats, jpg, url, hz, neural=None):
    ...
    try:
        ...
        if BLOB_TOKEN and now - _last_push["at"] >= BLOB_EVERY:
            ...
            # the public feed down with it. The ingress already carries frames,
            # telemetry and events for free, so the only thing that ever
            # needed publishing is where the ingress is. That is one small file,
...
async def begin():
    ...
    start_ingress(STATE.get("port", 4660))
