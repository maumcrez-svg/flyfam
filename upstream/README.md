...
if that service is unreachable the page falls back to a published ingress address.
...
So the fly opens a ingress-helper quick ingress instead and the socket carries
frames, telemetry and events for free. The only thing published anywhere is
where the ingress is — `site/web/live.json`, written when the address changes,
which the page reads. Quick ingress addresses are random and change every run,
